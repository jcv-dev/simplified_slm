...
Copied from matmulfreellm/mmfreelm/modules/fused_norm_gate.py
(originally from Mamba, Tri Dao).

Fuses RMSNorm + swish gating in a single Triton kernel for efficiency.
Falls back to pure PyTorch on CPU or when Triton is unavailable.
"""
...
def _rms_norm_swish_gate_ref(x, o, weight, bias=None, residual=None, eps=1e-6, prenorm=False):
    """Pure PyTorch reference implementation for RMSNorm + Swish Gate."""
    dtype = x.dtype
    x = x.float()
    o_float = o.float()
    ...
    normed = x * rstd * weight.float()
    if bias is not None:
        normed = normed + bias.float()
    out = normed * o_float * torch.sigmoid(o_float)
    out = out.to(dtype)
    if prenorm:
        return out, x.to(dtype)
    return out


# ---------------------------------------------------------------------------
# Triton-fused implementation (copied from matmulfreellm)
# ---------------------------------------------------------------------------
...
if _TRITON_AVAILABLE:
    ...
    def _layer_norm_fwd(
        ...
        with torch.cuda.device(x.device.index):
            _layer_norm_fwd_1pass_kernel[(M,)](
                x, o, y, weight, bias, residual, residual_out,
                mean, rstd,
    ...
    def _layer_norm_bwd(
        dy, x, o, weight, bias, eps, mean, rstd,
        ...
        with torch.cuda.device(x.device.index):
            _layer_norm_bwd_kernel[grid](
                x, o, weight, bias, y, dy, dx, do, _dw, _db,
    ...
    class LayerNormFn(torch.autograd.Function):
        @staticmethod
        def forward(
            ctx, x, o, weight, bias, residual=None, eps=1e-6,
        ...
        def backward(ctx, dy, *args):
            ...
            dx, do, dw, db, dresidual_in = _layer_norm_bwd(
                dy, x, o, weight, bias, ctx.eps, mean, rstd,
    ...
    def rms_norm_fn(x, o, weight, bias, residual=None, prenorm=False,
...
class FusedRMSNormSwishGate(nn.Module):
    ...
    Computes: RMSNorm(x) * o * sigmoid(o)

    Uses a Triton-fused kernel on CUDA (single kernel launch for both
    normalization and gating, with fused backward pass).
    Falls back to pure PyTorch on CPU or when Triton is unavailable.
